class BundlerBundle < ActiveRecord::Base
    ...
    def focals(files)
      files.map do |file_and_keyfile|
        ...
        begin
          ...
          resolution = img_info.match(/Resolution\s*:\s*(\d+)\s*x\s*(\d+)/)
          ccd_width = CCD_WIDTHS["#{make[1]} #{model[1]}"] if make && model
          if ccd_width.nil? && (jhead_width = img_info.match(/CCD width\s*:\s*(\w.*)mm/))
            ccd_width = jhead_width[1]
          end
          has_focal = !(focal.nil? || ccd_width.nil? || resolution.nil?)
          if resolution then
            res_x = resolution[1].to_i
            res_y = resolution[2].to_i
            if (res_x < res_y) then
              # Backwards aspect ratio
              tmp = res_x; res_x = res_y; res_y = tmp
            end
          end

          if has_focal then
            # Compute focal length area in pixels
            focal_pixels = res_x.to_f * (focal[1].to_f / ccd_width.to_f)
            [file, keyfile, 0, focal_pixels].join("\t")
          else
            [ file, keyfile ].join("\t")
          end
        rescue
  ...
  class List
    ...
    def keys
      #keyfiles = @src_files.map { |file| File.join(File.dirname(file), File.basename(file, File.extname(file))) + ".key" }
      #keyfiles.join("\n")
      @paths.map { |path| path[1] }.join("\n")
    end
  end

  def list
    List.new(self.photoset)
  end
  def controller
    BundlerController.new(self)
  end
end
